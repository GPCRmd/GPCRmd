 ...
 */
function createFlareplot(width, inputGraph, containerSelector,isGPCR){
    var w = width;
    var h = w;
    var rx = (w * 0.5) - 16;
    var ry = (w * 0.5) - 16;
        ...
        function create_bundle() {
            cluster = d3.layout.cluster()
                ...
                .sort(function(a, b) {
                    ...
                    //[!] --- Changes so that GPCR structure-based numbering (add +1 if insertion) gets properly sorted
                    if (/x/.test(a.key) && aRes.toString().length ==2){
                        aRes=aRes*10
                    }
                    if (/x/.test(b.key) && bRes.toString().length ==2){
                        bRes=bRes*10
                    }
                    //------
