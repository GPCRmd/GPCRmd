...
#  if method == 'POST':
    ...
    sdfnameref = get_file_name_submission("molecule",submission_id,molid,ref=True,ext="sdf",forceext=False,subtype="molecule")
    print(sdfnameref)
    uploadfile=open(os.path.join(submission_path,sdfnameref),'rb') #just put the string of SDF file into a file and open it.
    mol = open_molecule_file(uploadfile,filetype='sdf') # EL objeto mol es necesario para trabajar en RD KIT use this function with ARGUMENT! -> filetype='sdf' #MODIFIED BY ALEJANDRO, DO NOT KEEP THIS VERSION ON MERGE
    try:
        sinchi,code,msg = generate_inchi(mol,FixedH=False,RecMet=False)
        print("\nSINCHI",code)
        if code > 1:
 ...
 ##############################################################################################################################################################################################
        ...
        for comple in complexes:
            ...
            with closing(connection.cursor()) as cursor:
                ...
                elif len(compdbsinchi)>0: #check if inchi or inchikey already exist
                ...
                if flag==0: #if either the protein or the compound is not present, then, we are sure that a complex containing them does not exist.
                    ...
                    for i in resultlist:
                        if exactmatchtest_complex_exp(query_array,'complex',i)=='pass':
                            print('complex already exists, no need to record again')
                            exactest=1
        ...
        ##############################################################################################################################################################################################
                ...
                #doires=DyndbReferences.objects.filter(doi=comple[9]['DOI']) #warning, if comple[9]['DOI'] is '' it will return TRUE!
                ...
                if (len(doiresdoi)>0 and comple[9]['DOI']!=None) or (len(doiresurl)>0 and str(comple[9]['bindingdblink'])!='') or (len(doirespmid)>0 and comple[9]['pmid']!=None):
                    ...
                    if comple[9]['pmid']!=None:
                        ...
                        cursor.execute(
                            'INSERT INTO dyndb_references (doi, authors, url,dbname,pmid) VALUES (%s, %s, %s, %s, %s) RETURNING id', (comple[9]['DOI'],comple[9]['authors'], comple[9]['bindingdblink'], 'BindingDB',comple[9]['pmid'])
                        ) #the link is to the BindingDB entry, not the paper, but it is the closest thing the sdf file provides
                ...
                if complextype==1: #kd, binding
                    if len(DyndbBinding.objects.filter(id=complex_interaction_id))==0:
                        cursor.execute('INSERT INTO dyndb_binding (id, rvalue,units,description) VALUES (%s, %s, %s, %s)', (str(complex_interaction_id),str(kd),'nM',comple[9]['bindingdblink'])) #warning should i check if they already exist?
                    else:
                ...
                if complextype==2: #ec_50, efficacy
                    if len(DyndbEfficacy.objects.filter(id=complex_interaction_id))==0:
                        cursor.execute('INSERT INTO dyndb_efficacy (id, rvalue,units,description) VALUES (%s, %s, %s, %s)', (str(complex_interaction_id),str(ec_fifty),'nM',comple[9]['bindingdblink'])) #now, none referencecompound
                    else:
        ...
        ################################################################The sequece of this protein is NOT in the DB##########################
                        #maybe, that uniprot entry exists, but with a different sequence (a new mutant with respect to the cannonical not yet recorded in our DB)
                            ...
                            if len(DBproteins)>0:
                                ...
                                print('the cannonical',str(cannonical_id))
                                ...
                                if uniprot in GPCRlist: #if this uniprot code is a GPCR, find its id in receptor_id_protein
                                    ...
                                    cursor.execute(
                                        'INSERT INTO dyndb_protein (uniprotkbac,name,is_mutated,isoform,receptor_id_protein,id_uniprot_species) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id', (uniprot,namedata,'True',str(isoformid),receptor_protein_id,str(id_uniprot_species))
                                    )
        ...
        ##############################################################################################################################################################################################
                    ...
                    if len(DBcompound)>0:
                        ...
                        if comple[3]!='': #write chembleid when available
                            try:
                                cursor.execute('INSERT INTO dyndb_compound (name, iupac_name,pubchem_cid,sinchi,sinchikey,chembleid) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id', (names,iupac,pubchem_id,sinchi,sinchikey,str(comple[3])) )
                            except:
                                cursor.execute('INSERT INTO dyndb_compound (name, iupac_name,pubchem_cid,sinchi,sinchikey,chembleid) VALUES (%s, %s, %s, %s, %s, %s) RETURNING id', ('repeated_name_for_pubchemid'+str(pubchem_id),iupac,pubchem_id,sinchi,sinchikey,str(comple[3])) ) #two different pubchems ids share a name
                            ...
                            try:
                                ...
                                cursor.execute('INSERT INTO dyndb_compound (name, iupac_name,pubchem_cid,sinchi,sinchikey) VALUES (%s, %s, %s, %s, %s) RETURNING id', ('repeated_name_for_pubchemid'+str(pubchem_id),iupac,pubchem_id,sinchi,sinchikey) ) #two different pubchems ids share a name
                        ...
                        cursor.execute(
                            'INSERT INTO dyndb_references_compound (id_compound,id_references) VALUES (%s, %s)', (str(compound_id),str(creference_id))
                        )

                        #Recording the molecule...
                        try:
                            cursor.execute(
                            ...
                            for mol in DyndbMolecule.objects.filter(inchikey=str(molprop['inchikey'])):
                                ...
                                if col>maxcol:
                                    maxcol=col
                            cursor.execute(
                                'INSERT INTO dyndb_molecule (id_compound,description,net_charge,inchi,inchikey,inchicol,smiles,id) VALUES (%s, %s, %s,%s, %s, %s, %s, %s) RETURNING id', (str(compound_id),'Standard form',str(molprop['charge']),str(molprop['inchi']['inchi'][6:]),str(molprop['inchikey']),str(maxcol+1),str(molprop['smiles']),str(nextmol)) #perfect.
                            )                   
                        ...
                        cursor.execute(
                            'INSERT INTO dyndb_files_molecule (id_molecule,id_files,type) VALUES (%s, %s, %s)', (str(molecule_id),str(dyndbpngid),'2') #PNG file=type 2!
                            )
