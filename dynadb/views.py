...
for chunk in chunks[3:5]:
    ...
    ##############################################################################################################################################################################################
            ...
            if complextype==2: #ec_50, efficacy
                if len(DyndbEfficacy.objects.filter(id=complex_interaction_id))==0:
                    cursor.execute('INSERT INTO dyndb_efficacy (id, rvalue,units,description,reference_id_compound) VALUES (%s, %s, %s, %s, %s)', (str(complex_interaction_id),str(ec_fifty),'nM',comple[9]['bindingdblink'],'1'))
                else:
    ...
    ##################################################New Uniprot Code###################################################################
                            ...
                            namedata=namedata['RecName'][0]['Full'][0]
                            print(namedata)
                            print('other protein names?',namedataori['RecName'][0]['Full'])
