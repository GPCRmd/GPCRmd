...
def get_solvent_data(projid,solvent_types_d,wat_model,sol_num):
    ...
    if wat_model:
        ...
        db_wat_model=solvent_types_d["None"]
        water_is_filtered=False
    return (db_wat_model,water_is_filtered)
...
while i <= num_proj+1:
    ...
    if proj_query:
        ...
        if this_proj["published"]:
            ...
            for thisprot in prot_data:
                uniprotkbac=thisprot["_id"]
                prot_name=thisprot["header"]
            ...
            #SAVE DATA
            if save_data:
                (db_wat_model,water_is_filtered)=get_solvent_data(projid,solvent_types_d,proj_meta["WAT"],proj_meta["SOL"]):
                #Save protein
                ...
                dynobj,created_dynobj=CovidDynamics.objects.get_or_create(
                    ...
                    is_published=True,
                    solvent_type=db_wat_model,
                    solvent_is_filtered=water_is_filtered,
                )
                #Save molecules
                for lig in lig_li:
