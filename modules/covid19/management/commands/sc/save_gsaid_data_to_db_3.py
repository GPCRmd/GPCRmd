...
#        )
    ...
    def handle(self, *args, **options):
        ...
        def obtain_var_to_dfrow(df_path,varname):
            var_to_dfrow={}
            with open(df_path, 'r') as read_obj:
                csv_reader = csv.reader(read_obj)
                header=False
        ...
        isoid_to_isoDfRow= obtain_var_to_dfrow(isolates_path, "isolate_id")
        

        #mutations=pd.read_csv( os.path.join( gsaid_path, "mut_list.csv"))

        self.stdout.write(self.style.NOTICE("Reading WT sequences..."))
        all_genes={}   
        check=True
        if check:
            seqrow=0
            while True:
                seq=pd.read_csv( seq_path, nrows=1, skiprows=range(1, seqrow+1))
                if seq.empty:
                    break
                seqrow=seq.iloc[0]
                mut_descr=seqrow["mutation_description"]
                if mut_descr=="WT":
...
#                all_genes[nm]=seqid           
        ...
        genes_total_lines=55784471
        with open(genes_path, 'r') as read_obj:
            csv_reader = csv.reader(read_obj)
            header=False
            for row in tqdm.tqdm(csv_reader,total=genes_total_lines):
                if not header:
                    header=row
                else:
                    mygene= pd.DataFrame([row],columns=header)
                    if  mygene["bad"].item()=="True":
                        continue
                    seq_idx=int(mygene["seq_idx"].item())

                    #load_df_row_by_value(genes_path,"seq_idx",seq_idx)
                    myseq=seq.loc[seq["seq_idx"]==seq_idx]
                    mut_descr=myseq["mutation_description"].item()
                    ...
                    if save_to_db:
                        isolate_id=mygene["isolate_id"].item()
                        ...
                        isoDfRow=isoid_to_isoDfRow[isolate_id]
                        myisolate_df=pd.read_csv( isolates_path, nrows=1, skiprows=range(1, isoDfRow+1))
