...
def find_range_from_cons_pos(my_pos, gpcr_pdb):
    ...
    pos_range_l=int(gpcr_pdb[my_pos])
    pos_range_r=int()
    pos_range=""
    n=1
    pdb_positions = gpcr_pdb.values()
    while pos_range=="":
        if pos_range_l + n in pdb_positions:
            pos_range = (str(pos_range_l)+"-"+str(pos_range_l +n))
            break
        else:
            n+=1
        if n > len(pdb_positions):
            break
    return pos_range
 
def create_conserved_pos_list(gpcr_pdb,gpcr_aa, i,my_pos, cons_pos_li, add):
    ...
    while i < len(cons_pos_li):
        ...
        if my_pos_bw==cons_pos_bw:
            ...
            #cons_pos_li[i]+=["p"+str(i+add+1)]
            cons_pos_li[i][2]=pos_range
            my_aa=gpcr_aa[my_pos]
            if my_aa != cons_aa:
                cons_pos_li[i][0]=cons_pos+my_aa
        i+=1

def create_conserved_motif_list(gpcr_pdb,gpcr_aa,j,my_pos,motifs):
...
def find_missing_pos_in_motif(motifs, motname_li):
    ...
    for motname in motname_li:
        ...
        if num_nf ==0:
            #motifs_def.append([motname," , ".join(found),("m"+str(n+1))])
            ...
            #motifs_def.append([motname,(" , ".join(not_found)+" not found"),("m"+str(n+1))])
        n+=1
    return motifs_def
...
def obtain_prot_chains(pdb_name):
    ...
    chain_name_li=[]
    for chain in pdb.get_chains():  # Consider the case where there is more than 1 protein chain (error if more than 1, for the moment)
...
def obtain_predef_positions_lists(result,numbers,mol_sw,cons_classA,motifs):
    ...
    for pos in result:
        if pos[0] != "-": #Consider only num in the pdb
            ...
            if numbers[db_pos][1]: #If exist GPCR num for this position
                gpcr_pdb[numbers[db_pos][1]]=pos[0][2]
                gpcr_aa[numbers[db_pos][1]]=numbers[db_pos][0]
    gpcr_pdb_js=json.dumps(gpcr_pdb)            
    # Obtain list for the predefined views of important positions
    for my_pos in gpcr_aa:
        i=id_start_pos=0
        create_conserved_pos_list(gpcr_pdb, gpcr_aa,i,my_pos,mol_sw, id_start_pos)
        id_start_pos2=len(mol_sw)
        create_conserved_pos_list(gpcr_pdb, gpcr_aa,i,my_pos,cons_classA, id_start_pos2)
        j=0
        create_conserved_motif_list(gpcr_pdb,gpcr_aa,j,my_pos,motifs)
    find_missing_pos(mol_sw)
    find_missing_pos(cons_classA)
    motifs_def=find_missing_pos_in_motif(motifs, motname_li)
    return(gpcr_pdb_js,motifs_def)
                    ...
                    if len(chain_name_li)==1:
                        ...
                        if isinstance(checkpdb_res, tuple):
                            ...
                            if isinstance(result, list):
                                (gpcr_pdb_js,motifs_def)=obtain_predef_positions_lists(result,numbers,mol_sw,cons_classA,motifs)         
                                context={
